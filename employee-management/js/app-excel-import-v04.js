  ...
  function showColumnMapping(data) {
      ...
      for(let i = 0; i < data[0].length; i++){
         ...
         if(colValue == '' || colValue == null){
          ...
          if(date_array.includes(i)){
            dynamicTB += "<td><span style='color:#333;font-size: 9px'>--LAST ROW--</span><br/>" + getJsDateFromExcel(colValue) + "</td>"
          }else{
...
//                console.log("Pushing batch " + batch + " to server | start = " + start + " stop = " + stop);
  ...
  function updateProgress(import_id) {
      ...
      if(batch >= groups) {
          ...
          .done(function (response) {
            ...
            if(response.success == 1){
              ...
              $("#smx_progress-parsing").addClass("bg-warning");

              // list = '';
              error_message = '';
              error_count = '';
              textDiv = '';
              textDiv += '<div class="text-center">'
              ...
              if(response.hasOwnProperty("validation")){
                for (let i = 0; i < response.validation.length; i++) {
                  ...
                  error_count = response.error_count[response.validation[i]];
  
                  textDiv += '<button type="link" id="error_'+error_id+
                  '"" data-fn="'+error_message+'" class="btn btn-link errors" data-toggle="tooltip" data-placement="top" title="Click to view data with errors.">'+
                  error_message +' - ' + error_count + '</button><br>';
                }

                textDiv += '</div><div class="errorTable"></div>'
                $('#tableOutput').html("");
                $('#tableOutput').html(textDiv);
