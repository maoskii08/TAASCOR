...
class Data
{
    ...
    public function getIncompleteDetails(){
        ...
        try {
            ...
            $sql = "WITH duplicate_bank_account AS (
                        SELECT 
                            REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(atm_number, '-', ''), ' ', ''), '.', ''), '(', ''), ')', ''), '/', '') AS cleaned_atm_number,
                            COUNT(*) AS duplicate_count
                        FROM employee_salary w
                        INNER JOIN employee_list a ON a.employee_id = w.employee_id
                        WHERE w.atm_number IS NOT NULL 
                            AND TRIM(w.atm_number) <> '' 
                            AND a.status = 'Active'  
                        GROUP BY cleaned_atm_number
                        HAVING COUNT(*) > 1
                    )
                    SELECT 
                        a.employee_id, 
                        ...
                        w.atm_number,
                        REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(w.atm_number, '-', ''), ' ', ''), '.', ''), '(', ''), ')', ''), '/', '') AS cleaned_atm_number
                    ...
                    INNER JOIN duplicate_bank_account d ON d.cleaned_atm_number = 
                        REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(w.atm_number, '-', ''), ' ', ''), '.', ''), '(', ''), ')', ''), '/', '')
