...
$check(str_contains($script, 'Gemini processes guide questions.'), 'visible copy scopes external AI processing to guide questions');
$check(str_contains($script, 'Approved employee lookups stay inside TAASCOR'), 'visible copy states named lookups remain local');
$check(str_contains($script, 'Never enter salary, banking, government ID'), 'visible copy warns against restricted employee fields');
...
$check(str_contains($script, 'asksForForbiddenRecord'), 'prototype refuses requests for restricted employee fields');
$check(str_contains($script, 'I cannot reveal salary, banking, government ID'), 'restricted-field refusal is explicit');
$check(str_contains($script, "mode: 'gemini-secure-read'"), 'chat identifies itself as secure HRIS read mode');
$check(str_contains($script, 'Gemini + secure HRIS read'), 'visible secure read mode label is present');
$check(str_contains($script, 'tascaChatEmployeeCount'), 'approved roles receive an employee-count quick action');
$check(str_contains($script, "accessLevel() === '5'"), 'C&B receives a dedicated no-record boundary');
...
$check(str_contains($gateway, 'tasca_employee_intent($question)'), 'gateway routes approved employee reads locally');
$check(str_contains($gateway, "'data_mode' => 'local-read-only'"), 'gateway labels local database responses');
...
$check(str_contains($gemini, 'You have no direct access to databases'), 'Gemini system instruction denies direct record and tool access');
$check(str_contains($gemini, 'never included in this Gemini prompt'), 'Gemini never receives named employee records');
$check(str_contains($employeeReader, 'SELECT COUNT(*)'), 'employee reader exposes an allowlisted aggregate query');
$check(str_contains($employeeReader, 'ORDER BY e.full_name ASC LIMIT 5'), 'employee directory lookup is capped at five records');
$check(!preg_match('/\b(?:FROM|JOIN)\s+employee_salary\b/i', $employeeReader), 'employee reader never queries salary data');
$check(!preg_match('/\b(?:FROM|JOIN)\s+employee_govt_account\b/i', $employeeReader), 'employee reader never queries government identifiers');
$check(!preg_match('/\b(?:FROM|JOIN)\s+employee_details\b/i', $employeeReader), 'employee reader never queries contact or address details');
