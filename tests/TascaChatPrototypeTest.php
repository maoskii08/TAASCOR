...
$check(str_contains($footer, 'tasca-chat.js?v=20260801b'), 'TASCA script loads globally on authenticated pages');
...
$check(str_contains($script, 'Messages remain in memory and clear when this page reloads.'), 'visible copy explains memory-only conversation state');
...
$check(str_contains($script, "mode: 'guide'"), 'prototype identifies itself as local guide mode');
$check(str_contains($script, 'Guide mode only.'), 'visible privacy boundary is present');
...
$check(str_contains($script, "window.clearTimeout(state.responseTimer)"), 'new chat cancels a pending mock response');
...
$check(!str_contains($script, 'fetch('), 'mock prototype makes no external fetch request');
$check(!str_contains($script, 'XMLHttpRequest'), 'mock prototype makes no XMLHttpRequest');
$check(!str_contains($script, '$.ajax'), 'mock prototype makes no jQuery AJAX request');
