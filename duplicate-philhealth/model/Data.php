...
class Data
{
    ...
    public function getIncompleteDetails(){
        ...
        try {
            ...
            $sql = "WITH duplicate_philhealth AS (
                        SELECT 
                            REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(philhealth_number, '-', ''), ' ', ''), '.', ''), '(', ''), ')', ''), '/', '') AS cleaned_philhealth,
                            COUNT(*) AS duplicate_count
                        FROM employee_govt_account w
                        INNER JOIN employee_list a ON a.employee_id = w.employee_id
                        WHERE w.philhealth_number IS NOT NULL 
                            AND TRIM(w.philhealth_number) <> '' 
                            AND a.status = 'Active'  
                        GROUP BY cleaned_philhealth
                        HAVING COUNT(*) > 1
                    )
                    SELECT 
                        a.employee_id, 
                        ...
                        w.philhealth_number,
                        REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(w.philhealth_number, '-', ''), ' ', ''), '.', ''), '(', ''), ')', ''), '/', '') AS cleaned_philhealth
                    ...
                    INNER JOIN duplicate_philhealth d ON d.cleaned_philhealth = 
                        REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(w.philhealth_number, '-', ''), ' ', ''), '.', ''), '(', ''), ')', ''), '/', '')
